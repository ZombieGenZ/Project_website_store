...
function normalizeString(str) {
  ...
  async function GetAllAccountData(userid) {
    return new Promise((resolve, reject) => {
      database.query(`SELECT userid, username, email, money, revenue, avatarpath, Verify, createtime, permissionid, bio, penalty FROM Account`, [userid], (err, res) => {
        if (err) {
          reject(err);
        } else {
          if (res.length > 0) {
